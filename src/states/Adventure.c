...
void adventure_update() __banked {
  ...
  tile_x = (PLAYER.x + 4 + PLAYER.dir_x) >> 3;  // Add Left right Bias for Moving=True
  tile_y = (PLAYER.y + 7) >> 3;
  ...
  tile_x = (PLAYER.x + 4 - PLAYER.dir_x) >> 3;  // Remove LeftRight Bias to not stick
  ...
  if (ad_moving) {
    if (!(PLAYER.dir_x > 0 | PLAYER.dir_x<0 | PLAYER.dir_y> 0 | PLAYER.dir_y < 0)) {
      ad_moving = FALSE;
