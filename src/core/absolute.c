...
OAM_item_t      __at(SHADOW_OAM2_BASE_ADDRESS)                                           shadow_OAM2[40];        // 160 bytes
check_size(shadow_OAM2, 160);
palette_entry_t __at(SHADOW_OAM_PAGE_WITH_GAP + sizeof(shadow_OAM2))                     BkgPalette[8];          // 64 bytes
check_size(BkgPalette, 64);
UBYTE           __at(SHADOW_OAM_PAGE_WITH_GAP + sizeof(shadow_OAM2) +sizeof(BkgPalette)) vwf_tile_data[16 * 2];  // 32 bytes
